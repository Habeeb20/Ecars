...
import React, { useState } from 'react';
import { Search, ChevronDown, X, Loader2, SlidersHorizontal } from 'lucide-react';
import { toast } from 'sonner';
...
const SimpleCarCard = ({ car }) => (
  <div className="border border-gray-200 dark:border-gray-700 rounded-2xl p-4 bg-white dark:bg-gray-800 hover:shadow-lg transition-shadow cursor-pointer">
    ...
    <div className="text-sm text-gray-600 dark:text-gray-400 mt-3 flex flex-wrap gap-x-4">
      <span>{car.year || '—'}</span>
      <span>{car.mileage ? `${Number(car.mileage).toLocaleString()} km` : '—'}</span>
...
const SearchFilters = ({ variant = 'hero', className = '' }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = `${import.meta.env.VITE_BACKEND_URL}/cars/allcarsearch?${params.toString()}`;
      ...
      const data = await response.json();
      const cars = data.data || data.cars || data.results || [];
console.log('Fetched cars:', cars);
  ...
  return (
    <>
      {/* Main Search Bar */}
      <div className={`rounded-3xl shadow-xl p-6 bg-white dark:bg-gray-900 mb-40 ${className} lg:w-[1000px]`}>
        <form onSubmit={handleSubmit}>
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 text-indigo-600 hover:text-indigo-700 font-medium rounded-2xl hover:bg-indigo-50 dark:hover:bg-indigo-950 transition-colors"
            >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">Make</label>
              <select name="make" value={filters.make} onChange={handleChange} 
                className="w-full h-12 px-4 border border-gray-300 dark:border-gray-600 rounded-2xl dark:bg-gray-800 focus:ring-2 focus:ring-indigo-500">
                {makeOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
              </select>
            </div>

            <div>
              <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">Body Type</label>
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">Condition</label>
              <select name="condition" value={filters.condition} onChange={handleChange} 
                className="w-full h-12 px-4 border border-gray-300 dark:border-gray-600 rounded-2xl dark:bg-gray-800 focus:ring-2 focus:ring-indigo-500">
                {conditionType.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
              </select>
            </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-6 w-full h-14 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white font-semibold rounded-2xl flex items-center justify-center gap-3 text-lg shadow-lg hover:shadow-xl transition-all active:scale-[0.98]"
          >
      ...
      {isAdvancedOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <select name="priceRange" value={filters.priceRange} onChange={handleChange} 
                    className="w-full h-12 px-4 border border-gray-300 dark:border-gray-600 rounded-2xl dark:bg-gray-800">
            ...
            <div className="p-6 border-t dark:border-gray-700 flex gap-3">
              <button
                type="button"
                onClick={() => setIsAdvancedOpen(false)}
                className="flex-1 h-12 border border-gray-300 dark:border-gray-600 rounded-2xl font-medium"
              >
                Cancel
              </button>
              <button
                ...
                onClick={() => {
                  ...
                  handleSubmit({ preventDefault: () => {} }); // Trigger search
                }}
                className="flex-1 h-12 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl font-semibold"
              >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 rounded-3xl w-full max-w-6xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {Array.isArray(searchResults) && searchResults.length > 0 ? (
                  ...
                  <p className="text-center py-20 text-gray-500">No cars found matching your criteria.</p>
                )}
              </div>
            </div>
          </div>
