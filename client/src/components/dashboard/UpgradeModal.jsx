...
const UpgradeModal = ({ isOpen, onClose }) => {
  ...
  // Most common formats: RC1234567 or BN7654321
  const commonRegex = /^(RC|BN)\d{7}$/;

  // Also accept CAC/IT/NO format
  const trusteeRegex = /^CAC\/IT\/NO\s?\d{6,7}$/;

  if (!commonRegex.test(value) && !trusteeRegex.test(value)) {
    return 'Invalid format. Use RC1234567, BN7654321 or CAC/IT/NO 1234567';
  }

  if (value.startsWith('RC') || value.startsWith('BN')) {
    if (value.length !== 9) {
      return 'RC/BN number must be exactly 9 characters (e.g. RC1234567)';
    }
  }

  if (value.includes('CAC/IT/NO')) {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        {step === 'dealer' && (
          <div className="p-8 space-y-6">
            <h3 className="text-xl font-bold text-black">Dealer Registration</h3>
            <Input name="businessName" placeholder="Business Name"  required/>
            <Input name="businessRegistrationNumber" placeholder="CAC/Registration Number" required />
            <Input name="businessAddress" placeholder="Business Address" required />
        ...
        {step === 'service' && (
          <div className="p-8 space-y-6">
            ...
            <Input name="state" placeholder="State" required  />
            <Input name="lga" placeholder="LGA" required  />
            <Input name="phoneNumber" placeholder="Phone/WhatsApp"  required  />
