...
const AllCarPartsCategories = () => {
  ...
  const fetchAllCategories = async () => {
    ...
    try {
      let url = `${import.meta.env.VITE_BACKEND_URL}/carparts/categories?limit=200`; // increased limit
  ...
  // ── Fetch listings for clicked title ────────────────────────────────────
  const fetchItemsByTitle = async (title) => {
    ...
    try {
      ...
      console.error(err);
      toast.error('Failed to load listings for this category');
    } finally {
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
          <div>
            ...
            <p className="mt-2 text-gray-600 dark:text-gray-400">
              Browse every car part title — click to see all listings & seller details
            </p>
          </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-gray-50/50 dark:bg-gray-900/30">
              {chatMessages.length === 0 ? (
                ...
                chatMessages.map((msg) => (
                  <div
                    key={msg._id}
                    className={`flex ${msg.sender && msg.sender._id === localStorage.getItem('userId') ? 'justify-end' : 'justify-start'}`}
                  >
                    <div
                      className={`max-w-[75%] px-4 py-3 rounded-2xl ${
                        msg.sender && msg.sender._id === localStorage.getItem('userId')
