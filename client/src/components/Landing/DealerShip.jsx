...
const Dealership = () => {
  const [dealers, setDealers] = useState([]);              // All fetched dealers
  const [filteredDealers, setFilteredDealers] = useState([]); // Displayed after filter
  const [loading, setLoading] = useState(true);
  const [selectedDealer, setSelectedDealer] = useState(null);
  const [dealerCars, setDealerCars] = useState([]);
  const [dealerLoading, setDealerLoading] = useState(false);
  ...
  const brands = [
    ...
    'Mercedes-Benz', 'Audi', 'Lexus', 'Volkswagen', 'Hyundai',
  ];
  ...
  // Fetch all dealers once on mount
  useEffect(() => {
    fetchAllDealers();
  }, []);
  ...
  const fetchAllDealers = async () => {
    try {
      ...
      if (data.status === 'success') {
        const dealerList = data.data.dealers || [];
        setDealers(dealerList);
        setFilteredDealers(dealerList); // Initially show all
      } else {
        toast.error('Failed to load dealers');
      }
    } catch (err) {
      console.error(err);
      toast.error('Network error while loading dealers');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Apply filters (client-side when possible, fallback to server)
  const applyFilters = (currentFilters) => {
    const { brand, location, rating, type } = currentFilters;

    // If no filters → show all
    if (!brand && !location && !rating && !type) {
    ...
    const filtered = dealers.filter(dealer => {
      const dealerInfo = dealer.dealerInfo || {};
      const businessName = (dealerInfo.businessName || '').toLowerCase();
      const state = (dealerInfo.state || dealer.state || '').toLowerCase();
      const lga = (dealerInfo.lga || dealer.lga || '').toLowerCase();
      const locationText = `${state} ${lga}`.trim();
      const dealerBrands = (dealerInfo.brands || []).map(b => b.toLowerCase());
      const dealerType = dealerInfo.type?.toLowerCase() || '';
      const dealerRating = dealerInfo.rating || dealer.rating || 0;

      const matchesBrand = !brand || dealerBrands.includes(brand.toLowerCase());
      const matchesLocation = !location || 
        locationText.includes(location.toLowerCase()) ||
        businessName.includes(location.toLowerCase());
      const matchesRating = !rating || dealerRating >= Number(rating);
      const matchesType = !type || type.toLowerCase() === 'all types' || 
        dealerType.includes(type.toLowerCase());

      return matchesBrand && matchesLocation && matchesRating && matchesType;
    });

    setFilteredDealers(filtered);
    setLoading(false);

    // Optional: If you want server-side filtering for very large datasets, uncomment below
    /*
    try {
      const query = new URLSearchParams();
      ...
      if (type && type !== 'All Types') query.append('type', type);
      ...
      if (data.status === 'success') {
        ...
        setFilteredDealers([]);
        toast.error('No dealers match your filters');
      }
    } catch (err) {
      console.error(err);
  ...
  const openChat = () => {
    if (!isLoggedIn) {
      toast.error('Please login to chat with the dealer');
      window.location.href = '/login';
      return;
    }
    setShowChatModal(true);
            ...
            >
              <div className="h-48 overflow-hidden">
                <img 
                  ...
                  alt={dealer.dealerInfo?.businessName || dealer.firstName}
                  className="w-full h-full object-cover"
                  onError={(e) => { e.target.src = '/placeholder-dealer.jpg'; }}
                />
              </div>
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex text-yellow-400">
                    {[...Array(5)].map((_, i) => (
                      <Star 
                        key={i} 
                        className={`h-4 w-4 ${i < Math.floor(dealer.dealerInfo?.rating || dealer.rating || 0) 
                          ? 'fill-current' 
                ...
                <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <div className="flex items-center">
                    <MapPin className="h-4 w-4 mr-2" />
                    <span>
                      {dealer.dealerInfo?.state || dealer.state || 'N/A'},{' '}
                      {dealer.dealerInfo?.lga || dealer.lga || ''}
                    </span>
                  </div>
                </div>

                <div className="mt-4 flex flex-wrap gap-2">
                  {dealer.dealerInfo?.brands?.slice(0, 3).map((brand, index) => (
                    <span 
                      key={index}
                      className="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-xs rounded-full"
                    >
                  ...
                  {dealer.dealerInfo?.brands?.length > 3 && (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl max-w-4xl w-full my-8 shadow-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="p-6 border-b flex justify-between flex-shrink-0">
              <h2 className="text-2xl font-bold">
                {selectedDealer.dealerInfo?.businessName || `${selectedDealer.firstName} ${selectedDealer.lastName}`}
              </h2>
              <button onClick={() => setSelectedDealer(null)}>
                <X className="h-6 w-6" />
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6">
              ...
              <div className="flex items-center gap-2">
                <div className="flex text-yellow-400">
                  {[...Array(5)].map((_, i) => (
                    <Star 
                      key={i} 
                      className={`h-6 w-6 ${i < Math.floor(selectedDealer.dealerInfo?.rating || selectedDealer.rating || 0) 
                        ? 'fill-current' 
                        : 'fill-none stroke-current'}`} 
                    />
                  ))}
                ...
                <span className="text-xl font-bold">
                  {selectedDealer.dealerInfo?.rating?.toFixed(1) || 'N/A'}
                </span>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <p><strong>Business Name:</strong> {selectedDealer.dealerInfo?.businessName || 'N/A'}</p>
                <p><strong>First Name:</strong> {selectedDealer.firstName}</p>
                <p><strong>Last Name:</strong> {selectedDealer.lastName}</p>
                <p><strong>Email:</strong> {selectedDealer.email}</p>
                <p><strong>Phone:</strong> {selectedDealer.phoneNumber}</p>
                <p><strong>Business Address:</strong> {selectedDealer.dealerInfo?.businessAddress || 'N/A'}</p>
                <p><strong>State:</strong> {selectedDealer.dealerInfo?.state || selectedDealer.state || 'N/A'}</p>
                <p><strong>LGA:</strong> {selectedDealer.dealerInfo?.lga || selectedDealer.lga || 'N/A'}</p>
                <p><strong>Verified:</strong> {selectedDealer.dealerInfo?.verified ? 'Yes' : 'No'}</p>
                <p><strong>Featured:</strong> {selectedDealer.dealerInfo?.isFeatured ? 'Yes' : 'No'}</p>
                <p><strong>Featured Until:</strong> {selectedDealer.dealerInfo?.featuredUntil 
                  ? new Date(selectedDealer.dealerInfo.featuredUntil).toLocaleString() 
                  : 'N/A'}</p>
              </div>

              <h3 className="text-xl font-bold mt-6">Brands</h3>
              <p className="text-gray-600 dark:text-gray-400">
                {selectedDealer.dealerInfo?.brands?.join(', ') || 'No brands listed'}
              </p>
              ...
              {dealerLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {dealerCars.map(carItem => (
                    <div key={carItem._id} className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4">
                      <img 
                        src={carItem.images?.[0] || '/placeholder-car.jpg'} 
                        alt={carItem.title}
                        className="w-full h-32 object-cover rounded-lg mb-2"
                        onError={(e) => { e.target.src = '/placeholder-car.jpg'; }}
                      />
                      <p className="font-bold">{carItem.title || 'Untitled'}</p>
                      <p className="text-sm text-indigo-600">
                        ₦{Number(carItem.price || 0).toLocaleString()}
                      </p>
                    </div>
                  ))}
                </div>
            ...
            <div className="p-6 border-t flex-shrink-0">
              <button 
                onClick={openChat}
                className="w-full py-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold rounded-xl flex items-center justify-center gap-3 hover:shadow-lg transition-shadow"
              >
                <MessageCircle className="h-6 w-6" />
                Chat with Dealer
              </button>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl max-w-lg w-full my-8 shadow-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-8">
              <p className="text-gray-600 dark:text-gray-300 mb-4">
                Send a message to {selectedDealer.dealerInfo?.businessName || 'the dealer'}
              </p>
              <textarea
                rows="8"
                value={chatMessage}
                onChange={(e) => setChatMessage(e.target.value)}
                placeholder="Hello, I'm interested in your dealership..."
                className="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 resize-none focus:ring-2 focus:ring-indigo-500 outline-none"
              />
            </div>
            ...
            <div className="p-6 border-t flex-shrink-0">
              <button 
                onClick={sendMessage}
                disabled={chatLoading}
                className="w-full py-4 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold rounded-xl flex items-center justify-center gap-3 disabled:opacity-50 transition"
              >
                {chatLoading ? (
