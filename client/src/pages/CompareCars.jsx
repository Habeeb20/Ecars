...
const CompareCars = () => {
    ...
    try {
      ...
      const queryParams = new URLSearchParams({
        ...
        ...(yearTo && { yearTo: yearTo.trim() }),
                ...(minPrice && { minPrice: minPrice.trim() }),     // NEW
        ...(maxPrice && { maxPrice: maxPrice.trim() }),  
      });
  ...
  const filteredCars = allCars.filter(car => {
    ...
    if (minPrice && !isNaN(carPrice)) {
      matchesPrice = matchesPrice && carPrice >= Number(minPrice);
    }
    if (maxPrice && !isNaN(carPrice)) {
      matchesPrice = matchesPrice && carPrice <= Number(maxPrice);
    }
        ...
        <div className="mb-8 bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="relative flex items-center gap-2">
              ...
              className="h-12 px-4 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-indigo-500"
            />
            <input
              type="number"
              value={maxPrice}
