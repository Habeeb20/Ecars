...
import { useAuth } from '../contexts/AuthContext';
import { motion } from 'framer-motion';
import { ShieldCheck, FileSearch, Key, Calendar,Phone,ImageIcon  } from 'lucide-react';
const SellCar = () => {
  const { user } = useAuth();
  const [formData, setFormData] = useState({
    make: '',
  ...
  const handleSubmit = async (e) => {
    ...
    const { make, model, year, price, description, phoneNumber } = formData;
    if (!make || !model || !year || !price || !description || !phoneNumber || files.length === 0) {
      toast.error('Please fill all fields and upload at least one photo');
      return;
    }
    ...
    try {
      ...
      const apiRes = await fetch(`${import.meta.env.VITE_BACKEND_URL}/cars`, {
        method: 'POST',
        headers: {
      ...
      if (data.status === 'success') {
        ...
        // Reset form
        setFormData({ make: '', model: '', year: '', price: '', description: '', phoneNumber: '' });
        setFiles([]);
        previews.forEach(URL.revokeObjectURL);
        setPreviews([]);
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-black py-16 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-700 mb-12 p-8">
          <h2 className="text-3xl font-black text-gray-900 dark:text-white mb-8 text-center">
            What to Bring With You
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {/* Your ID */}
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-700 dark:to-gray-800 p-6 rounded-2xl shadow-lg">
              <div className="flex justify-center mb-4">
                <div className="p-4 bg-blue-600 rounded-full">
                  <ShieldCheck className="w-8 h-8 text-white" />
                </div>
              </div>
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4 text-center">Your ID</h3>
              <ul className="space-y-3 text-gray-700 dark:text-gray-300 text-sm">
                <li className="flex items-start">
                  <Check className="w-4 h-4 text-green-500 mr-2 mt-1 flex-shrink-0" />
                  <span>Valid Nigerian driving licence or passport</span>
                </li>
                <li className="flex items-start">
                  <Check className="w-4 h-4 text-green-500 mr-2 mt-1 flex-shrink-0" />
                  <span>Two original documents providing your current address (dated within the last three months, for example bank statement, council tax bill or utility bill)</span>
                </li>
                <li className="flex items-start">
                  <Check className="w-4 h-4 text-green-500 mr-2 mt-1 flex-shrink-0" />
                  <span>If you have outstanding finance, you'll need to bring proof of settlement</span>
                </li>
              </ul>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-4 italic">Please Note: We do not accept mobile phone bills as proof of address.</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-700 dark:to-gray-800 p-6 rounded-2xl shadow-lg">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4 text-center">Your Car's ID</h3>
              <ul className="space-y-3 text-gray-700 dark:text-gray-300 text-sm">
                <li className="flex items-start">
                  <Check className="w-4 h-4 text-green-500 mr-2 mt-1 flex-shrink-0" />
                ...
                <li className="flex items-start">
                  <Check className="w-4 h-4 text-green-500 mr-2 mt-1 flex-shrink-0" />
                  <span>Proof of Ownership / Registration Document</span>
                </li>
                <li className="flex items-start">
                  <Check className="w-4 h-4 text-green-500 mr-2 mt-1 flex-shrink-0" />
                ...
                <li className="flex items-start">
                  <Check className="w-4 h-4 text-green-500 mr-2 mt-1 flex-shrink-0" />
                  <span>Any other bills or receipts relating to your car</span>
                </li>
              </ul>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-4 italic">Please Note: The car must be registered to you as the legal owner, at your address, for at least 3 months.</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-700 dark:to-gray-800 p-6 rounded-2xl shadow-lg">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4 text-center">Your Car's Extras</h3>
              <ul className="space-y-3 text-gray-700 dark:text-gray-300 text-sm">
                <li className="flex items-start">
                  <Check className="w-4 h-4 text-green-500 mr-2 mt-1 flex-shrink-0" />
                ...
                <li className="flex items-start">
                  <Check className="w-4 h-4 text-green-500 mr-2 mt-1 flex-shrink-0" />
                  <span>Wheel nut keys</span>
                </li>
                <li className="flex items-start">
                  <Check className="w-4 h-4 text-green-500 mr-2 mt-1 flex-shrink-0" />
                  <span>Anything else that belongs with the car</span>
                </li>
              </ul>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-4 italic">Please Note: We do not accept mobile phone bills as proof of address.</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-700">
          ...
          <form onSubmit={handleSubmit} className="p-8 lg:p-12 space-y-8">
            ...
            <div className="relative">
              <label className="block text-lg font-semibold text-gray-800 dark:text-gray-200 mb-3">
                <Phone className="inline w-6 h-6 mr-2 text-blue-600" />
                Phone Number
              </label>
              <input
                type="tel"
      ...
      {showLoginModal && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <motion.div
            initial={{ scale: 0.8, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="bg-white dark:bg-gray-800 rounded-3xl max-w-lg w-full p-8 shadow-2xl text-center"
          >
            <div className="flex justify-center mb-6">
              <div className="p-6 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-full">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-8">
              You need to be logged in to list your car and track your listing.
            </p>
            <div className="flex flex-col gap-4">
              <button
                onClick={() => window.location.href = '/login'}
                className="py-4 px-8 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all"
              >
                Log In Now
              </button>
              <button
                onClick={() => setShowLoginModal(false)}
                className="py-4 px-8 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-semibold rounded-xl hover:bg-gray-300 dark:hover:bg-gray-600 transition-all"
              >
                Cancel
              </button>
            </div>
